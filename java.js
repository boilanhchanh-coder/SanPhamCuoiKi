window.addEventListener('load', function(){
    let u = document.getElementById("regUser");
...
function dangbai(){
    ...
    let noidung = document.getElementById("noidungbaiviet").value;
    let status = document.querySelector(".status").value;
    let baiviet = getData("baiviet");
    let now = new Date();
    let thoigian = now.toLocaleString();
    if(!tieude || tieude === "Nhập tiêu đề"){
        showError("tieude", "Nhập tiêu đề")
        return;
    }
    if(!chude || chude === "Nhập chủ đề"){
        showError("chude", "Nhập chủ đề")
        return;
    }
    let baivietmoi = {
        tieude: tieude,
